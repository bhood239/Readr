...
const UserList = (props) => {
  ...
  const isFollowing = (userId) => {
    return currentUser.following_list?.some(
      (followingUser) => followingUser.id === userId
    );
  };
  const isFollower = (userId) => {
    return currentUser.followers_list?.some(
      ...
      {users &&
        users.map((user) => {
          const followButtonText =
            buttonStates[user.id]?.follow ??
            (isFollowing(user.id) ? "Unfollow" : "Follow");
          const removeFollowerButtonText =
            buttonStates[user.id]?.removeFollower ??
            (isFollower(user.id) ? "Remove Follower" : null);
